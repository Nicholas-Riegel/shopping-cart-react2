      ...
      {items.map( (x, i) => (
        <div key={i} className='item_div'>
          <div className="title_div">
            <h3>{x.title}</h3>
            <img src={`${process.env.PUBLIC_URL}/pictures/` + x.image} alt={x.title} />
          </div>
            ...
            <div className="buttons_div">
              ...
              <input
                ...
                onChange={(e) => setItems(
                  items.map(
                    y => y.id === x.id
                      ? { ...y, count: (parseInt(e.target.value) || e.target.value === "0" ? parseInt(e.target.value) : '') }
                      : y
                  )
                )}
              />
